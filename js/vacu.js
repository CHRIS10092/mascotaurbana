...
const listar_articulos = e => {
	...
	let url = '../controladores/vacu/listar_articulos.php';
	http(url,'POST',null)
	    .then(res => {
...
const registrar_venta = () => {
	...
	let url = '../controladores/vacu/registrar_venta.php';
...
function guardar() {
	...
	$.ajax({
		url:'../controladores/historial/crear.php' ,
		type:'POST' ,
		data:datosq ,
		contentType: false,
        processData: false,
	...
	.done(function(r) {
		...
		if (r==1) {
			...
			setTimeout(function() {
				window.location.href="../app/vacu.php";
			}, 1000);
		}else{
