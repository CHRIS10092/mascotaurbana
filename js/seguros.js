...
const listar_articulos = e => {
	...
	let url = '../controladores/seguros/listar_articulos.php';
	http(url,'POST',null)
	    .then(res => {
...
const registrar_venta = () => {
	...
	let url = '../controladores/seguros/registrar_venta.php';
...
function guardar() {
	...
	$.ajax({
		url:'../controladores/seguros/crear.php' ,
		type:'POST' ,
		data:datosq ,
		contentType: false,
        processData: false,
	...
	.done(function(r) {
		//console.log(r);
		if (r==1) {
			alertify.success("Registro de Seguro Exitosa");
			setTimeout(function() {
				window.location.href="../app/seguros.php";
			}, 1000);
		}else{
			alertify.error("No se pudo Registrarel Seguro");
		}
	})
