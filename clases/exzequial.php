...
class exzequial extends config
{
    ...
    public function listar_articulos($id_empresa)
    {
       ...
       SUM(DISTINCT inv_stock) AS stock from tbl_inventarios inv ,tbl_empresas e , inv_tblsubcategoria c WHERE inv.idempresa=:id_empresa

         AND e.emp_id=inv.idempresa AND inv_valorpvp > 0 AND inv_stock> 0   AND c.id=inv.idsubcategoria
         AND c.id='33' GROUP by inv_id";
    ...
    public function insertarexzequial($datos)
    {

        $inv_sql  = "INSERT INTO tbl_exequiales(exe_descripcion,idmascota,exe_fecha,exe_fecha_proxima,exe_idarticulo) VALUES (?,?,?,?,?)";
